...
@lesson[#:title "Drawing the World"
        #:duration "40 minutes"]{
          @itemlist/splicing[
             @item{Now that we've got our world structure, we need to know how to draw it. Turn to page 23, and fill in your @code{START} world at the bottom. Once you have that done, do a simple sketch of your @code{START} world. According to your world struct, where should everything be when the game starts? @tag[pedagogy]{Take one minute and do that now. Be sure to count down!}}
             ...
             @item{In the chart on page 23, figure out which image goes on top, which goes second, and so on. Make a list from top to bottom in the left column, and then write each image's coordinates in the right column.@tag[pedagogy]{ Take two minutes to do so.}}
             @item{@tag[pedagogy]{Excellent! }Let's set up one more example, so that when we get into writing our function that draws the world, we're ready to go. On page 24 there's a nearly identical page. You've already written a @code{START} world, which has everything where it will be when the game starts. Now do the same for a world called @code{NEXT}. This world is where everything is ONE SECOND after the game starts. @tag[pedagogy]{Again, you have one minute to fill in a world struct, sketch the NEXT world. Go!}}
             ...
             @item{On page 25, there's a place for you to write the contract. Below that, we see a sort of staircase pattern using @code{put-image}, just like we did in Ninja World.}
