...
@itemlist/splicing[
          ...
          @item{My favorite shape in the whole world is a triangle, and my favorite color is green. I LOVE making solid green triangles! But right now, I have to type out so much code to do that! I need to write the triangle function, then a size, then tell the computer that I want it solid and green. I wish there was a shorter way, another function called gt
...
(EXAMPLE (gt 50) (triangle 50 "solid" "green"))}}
         @tag[pedagogy]{@item{Can someone write another example for me?}}
          ...
          @item{Now we can write the code -- instead of an EXAMPLE we'll use define. After that, we're just going to copy everything from our examples except the stuff that we circled. What do you think we'll write instead? We'll use the name we wrote down: size. @tag[pedagogy]{Go character-by-character with the students, asking them if both examples have an open paren, the name "gt", etc...} @embedded-wescheme[#:id "Fast Fucntions"
