...
(define japan (put-image (circle 50 \"solid\" \"red\")
    ...
    @item{The first thing in @code{put-image}'s domain is the Image we want to place. What is our image? @tag[pedagogy]{A circle!}
                             
          @code{(put-image (circle 50 "solid" "red") ___ ___ ________)}}
    @item{What are the next two things in the domain? @tag[pedagogy]{Numbers!} They represent the x and y coordinates for where we want to place the image of the circle. What did we determine were the coordinates for the center of the circle? Let's fill them in!
