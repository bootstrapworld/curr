...
@lesson[#:title "onscreen? 1.0" 
 ...
 @itemlist/splicing[
           @tag[pedagogy]{@item{Draw a rectangle on the board, to represent the screen. Then turn students' attention to the computers, and have them analyze the last function in the code, called @code{onscreen?}.}}
            ...
            @tag[pedagogy]{@item{Who would like to act out @code{protect-left}? Take a volunteer.}}
            ...
            @item{So now we have a function that will protect Sam on the left side of the screen. But at the moment, @code{onscreen?} doesn't know how to use that function. We need to change the body of @code{onscreen?} so that it can talk to @code{protect-left}:@linebreak[]
                  @code{(define (onscreen? x)@linebreak[] (protect-left x))}}
