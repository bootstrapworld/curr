 ...
 @code[#:multi-line #t]{(define (update-world w)
	(cond
		...
		[else  (make-world  (+ (world-dogX w) 10) 
				       (world-rubyX w)
				       (world-catY w))]))}
@item{We still want our original code to be there - it's now going to be used in the else clause, because when @code{dogX} is not off the right side of the screen we want the world to update normally. @tag[pedagogy]{Walk the students through the transformation.}}
