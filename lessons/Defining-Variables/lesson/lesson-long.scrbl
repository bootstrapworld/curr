...
@lesson[#:title "Defining Variables"]{
        ...
        @tag[selftaught]{@item{@embedded-wescheme[#:id "Defining Variables"
        ...
        @tag[selftaught]{@item{The following is a bare-bones, totally broken game. It doesn't DO anything...YET!}
                         @item{@embedded-wescheme[#:id "Defining Variables"   
                                                 #:public-id "undue-rival-cream-plane-fluid"]}}
        ...
        @item{This kind of name, which is just a shortcut for some value like a Number, String, or Image, is also called a @bold{variable}. You've seen other names too, like @code{+} and @code{string-length} that are the names of functions. You'll name your own functions soon.}
        @item{@exercise{Change the value of this variable from @code{"My Game"} to YOUR title. Then click Run, and see if the game is updated to reflect the change.}}
