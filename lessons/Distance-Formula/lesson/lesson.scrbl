...
@lesson[#:title "The Distance Formula"
        ...
        @itemlist/splicing[@item{That's because you haven't written the code to calculate the distance in two dimensions! All you have is something that tells you the length in the x- and y-dimension.} 
                   @item{Ancient civilizations had the same problem: they also struggled with distance in two dimensions! An ancient Greek named Pythagoras finally figured it out...maybe you've heard of him?}
                   @pedagogy{@item{Draw normal Cartesian coordinate plane, with a point on each axis. Label the coordinates (3,0) and (0,4).}}
                   @tag[selftaught]{@item{Draw a coordinate plane. Label the coordinates (3,0) and (0,4).}}
                   @item{@bitmap{images/3004ABCgraph.png}
                   ...
                   @item{To make our lives easier, we can use the function @code{line-length}, and pass in the two x-coordinates (zero and three). Just like we did with the butterfly example, we can write functions that call each other for help!}
                   @item{In our example, @code{(line-length 0 3)}is 3 and @code{(line-length 4 0)} is 4, but we still don't know C. What is the length of C?}
                   @pedagogy{@item{Pass out Pythagorean Proof materials to each group, and have them review all of their materials:@itemlist[@item{A large, white square with a smaller one drawn inside}@item{Four gray triangles, all the same size}]}}
                   ...
                   @item{Now we have four triangles, each with a side A, B and C. We also have two squares: the inner square, whose sides are a C, and the outer square, whose sides are (A+B).@bitmap{images/csquared.png}}
                   @pedagogy{@item{Raise your hand if you know how to find the area of a square. Take a volunteer.}}
                   @item{@think-about[#:question "What's the area of the white, inner square?" #:answer "white space = C squared"]}
                   @tag[(selftaught pedagogy)]{@item{Watch this 
                                                   @(resource-link #:path "images/Pythag_anim.gif"
                                                              #:label "animation")
                                                   @pedagogy{Teacher's, draw this on the board.}
                   ...
                   @item{Since we didn't change the size of the outer square, and all we did was move stuff around inside it, we know there is still the same amount of white space as there was before - it's just broken into two parts now. So the area of the whitespace is still C@superscript{2}.}
                   ...
                   @item{What about the bigger white square? We know that both of its sides are of length B, so its area must be B@superscript{2}. So now we have two ways of writing the area of the white space:}
                   @item{@bitmap{images/absquare.png}So whitespace = C@superscript{2} = A@superscript{2} + B@superscript{2}}
                   ...
                   @item{Okay, so we know that C@superscript{2} is 25...but remember, we want C by itself. What is the square root of 25? It's five!}
                   ...
                   @item{Turn to @worksheet-link[#:page 27 #:name "Distance-Formula-With-Numbers"] of your workbook - you'll see the formula written out. @tag[(pedagogy group)]{Let's do this one together, since it's been a long time since we've done Circles of Evaluation as a class.}}
