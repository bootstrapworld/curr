...
@itemlist/splicing[
          ...
          @item{Now it's time to write some examples. Let's look at the first example "bc" drew on the board, for @code{(bc 50)}. @think-about[#:question "What shape did they draw? What color? What size? How would you write the code to draw that shape?" #:answer @list{@tag[pedagogy]{(write on the board):} @list{@code{ (EXAMPLE (bc 50) (circle 50 @code[#:string "\"solid\" \"blue\""]@code{))}}}}]}
