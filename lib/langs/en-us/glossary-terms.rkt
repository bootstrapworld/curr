...
|#
  ...
  (("accessor functions" "functions to extract values from a data structure")
    ...
    ("categorical data" "values or classifications that do not follow a numerical progression, and are no subject to the laws of arithmetic")
    ...
    (("data row" "data rows") "data that has been gathered about a particular individual in a sample")
    ...
    (("histogram" "histograms") "A display of a quantitative variable that uses the horizontal axis to mark of regular intervals (called bins) encompassing all the values in a dataset. A bar over each bin shows the number of percent of data values in that interval, to indicate frequency in the vertical axis.")
    ...
    ("interquartile range" "The interquartile range (IQR) is a measure of variability, and is calculated by subtracting the first quartile (Q1) from the third quartile (Q3)")
    ...
    (("outlier" "outliers") "an observation point that is distant from other observations, perhaps due to experimental error or natural variability.")
    ...
    ("quartiles" "Numbers that divide a a data set into four, equally-sized groups. The lowest quartile (Q1) divides the bottom half of the data. Q2 (the median) divides the entire sample. Q3 divides the second highest from the highest group.")
    ...
    (("shape") "The aspect of a dataset that telling which values are more or less common")
    ...
    ("statistical range" "the distance between the lowest and highest values in a data set")
