...
|#
  ...
  (("accessor functions" "functions to extract values from a data structure")
    ...
    (("bar chart" "bar charts") "a visualization of categorical data in which each bar has a height corresponding to the count or proportion of values in a given category.")
    ...
    (("box plot" "box plots") "The box plot (a.k.a. box-and-whisker pot) is a way of displaying the distribution of data based on the five-number summary: minimum, first quartile, median, third quartile, and maximum")
    ...
    (("correlation" "correlations") "A number that summarizes the linear relationship between two quantitative variables by reporting its strength and direction")
    ("data block" "Code that lists the name, constructor(s), and field(s) of a data structure")
    (("data row" "data rows") "part of a table showing information about a particular individual in a sample")
    ...
    (("histogram" "histograms") "A display of a quantitative variable that uses the horizontal axis to mark of regular intervals (called bins) encompassing all the values in a dataset. A bar over each bin shows the number or percent of data values in that interval, with frequency indicated in the vertical axis.")
    ...
    ("interquartile range" "The interquartile range (IQR) is a measure of spread, and is calculated by subtracting the first quartile (Q1) from the third quartile (Q3)")
    ...
    (("line of best fit" "lines of best fit") "A straight line that best represents the data on a scatter plot, assuming the form is linear. Also called the 'regression line'.")
    ("linear regression" "The process of modeling the relationship between two quantitative variables with a line that makes the best predictions of responses (y values) given explanatory (x) values.")
    (("list" "lists") "a data structure containing a sequence of values")
    ("mathematical expression" "a computation in arithmetic")
    ("mean" "reports the center of a quantitative data set by dividing the sum of all the values by the number of values")
    ...
    ("median" "the middle of a quantitative data set: half the values are above it, and half are below")
    ("mode" "the most commonly appearing value in a (quantitative or categorical) data set")
    ...
    (("outlier" "outliers") "an observation point that is distant from other observations, perhaps due to experimental natural variability or measurement error.")
    ("piecewise function" "a function that computes different expressions based on its input")
    (("pie chart" "pie charts") "A display of categorical data that shows the relative proportion (or %) of a column's values that fall into each category. The greater the proportion, the larger the pie slice.")
    ...
    (("quartile" "quartiles") "Numbers that divide a a data set into four, equally-sized groups. The lowest quartile (Q1) is the middle of the bottom half of the data. Q2 (the median) is the middle of the entire sample. Q3 is the middle of the top half of of the data.")
    ...
    (("r-squared") "A number between 0 and 1 that measures the strength of a linear relationship between two variables")
    ...
    (("scatter plot" "scatter plots") "A display of the relationship between two quantitative variables.")
    ...
    (("table" "tables") "A data structure that with one row for each individual in a sample, and one column for each variable studied. Also includes an extra, initial row that identifies the names of the variables.")
