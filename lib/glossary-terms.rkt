...
(define glossary-terms-dictionary
  '(("accessor functions" "functions to extract values from a data structure")
    ...
    ("data structure" "A description of a container for values, which are stored in named fields")
    ...
    ("examples block" "Code that lists examples starting with 'examples:' and ending with 'end'")
    ("expression" "a computation written in the rules of some language (such as arithmetic, code, or a Circle of Evaluation)")
    ("field" "A component of a data structure that has a name and holds a single value of a specified datatype")
    ...
    ("handler" "A connection between an event (like a tick or keypress) and a function")
    ...
    ("instance" "A specific constructed value of a data structure that contains other values in its fields.")
    ("instances" "A collection of single instances, or also used to describe illustrative instances that we use to provide examples of a data structure")
