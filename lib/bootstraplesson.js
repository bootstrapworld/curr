...
function isNewest() {
  // add the "obsolete" banner
  function displayOutOfDate(loadEvent) {
    if(loadEvent.target.status !== 200) return; // not a successful load
  ...
  // does a particular URL return a page?
  function UrlExists(url){
    var request = new XMLHttpRequest();
    request.addEventListener("load", displayOutOfDate)
    request.open('HEAD', url);
    request.send();
  }

  // get the current year
  var current_year = new Date().getFullYear(), url = window.location.href;
  var current_season = (new Date().getMonth() > 7)? "fall" : "spring"; // after August, it should be fall
  var next_season = current_season == "fall"? "spring" : "fall";
  var base_url = "https://www.bootstrapworld.org/materials/";

  // if we're in fall of this year, check to see if spring of next year exists
  // if we're in spring of this year, check to see if fall of this year exists
  if(current_season == "spring") {
    console.log('curent season is spring'+current_year+'. looking for ', base_url+"fall"+current_year);
    if(url.indexOf(base_url+next_season+current_year) == -1) UrlExists(base_url+"fall"+current_year);
  } else {
    console.log('current season is fall'+current_year+'. looking for ', base_url+"spring"+(current_year+1));
    if(url.indexOf(base_url+next_season+(current_year+1)) == -1) UrlExists(base_url+"spring"+(current_year+1));
  }
}
...
function showGroup(){
  window.open("https://discourse.bootstrapworld.org", "_blank");
}
