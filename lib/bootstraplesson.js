...
function showSlides() {
  ...
  var cardNodes = document.getElementsByClassName('student'), //,.BootstrapPageTitle
      cardsRaw     = [].slice.call(cardNodes).filter(function(card) {
      	return card.getElementsByClassName('BS-Skip-Slide').length === 0;}),// convert NodeList to Array, remove slides to be skipped
      cards = cardsRaw.forEach(function(card) {
      	card.body.appendChild(card.content)
      })
