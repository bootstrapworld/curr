...
function initializeCards(){
  ...
  function drawLesson(lesson){
    ...
    var lessonUL = lesson.getElementsByClassName('lesson')[0];
    lessonUL.style.left = (-lesson.currentCard * width) + 'px';
  }
  // get all the lessons and cards
  var lessons = document.getElementsByClassName('LessonBoundary');
  var lesson = lessons[0],
      list = lesson.getElementsByClassName('lesson')[0];
  // compile all cards into a single lesson
  for(var i=0; i<lessons.length; i++){
    var cards = lessons[i].getElementsByClassName('lessonItem');
    if(i>0){lessons[i].style.display='none'; }
