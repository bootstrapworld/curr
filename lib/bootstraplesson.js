...
function isNewest() {
  ...
  var rest_url = "/"+url.slice(url.indexOf("courses"));
  ...
  if(url.indexOf(base_url+"spring"+current_year) > -1) {
    console.log('curent url is spring. looking for ', base_url+"fall"+current_year+rest_url);
    UrlExists(base_url+"fall"+current_year+rest_url);
  } else {
    console.log('current url is fall. looking for ', base_url+"spring"+(current_year+1)+rest_url);
    UrlExists(base_url+"spring"+(current_year+1)+rest_url);
  }
}
...
function attachCodeMirror(){
  ...
  forEach(codeSnippets, function(idx, textarea){
    if(textarea.className == "racket"){
      ...
      CodeMirror.fromTextArea(textarea, {mode:"pyret", readOnly: "true", cursorBlinkRate: 0, theme: "pyret"});
    }
  });
