...
function attachCodeMirror(){
  ...
  forEach(codeSnippets, function(idx, textarea) {
    var mode =  (textarea.className == "racket")? "scheme2" 
              : (textarea.className == "pyret")?  "pyret"
              : false;
    CodeMirror.fromTextArea(textarea, {mode: mode, readOnly: "true", cursorBlinkRate: 0});
  });
  ...
  forEach(codeSnippets, function(idx, tt){
    ...
    var mode = (tt.className == "racket")? "scheme2" 
            : (tt.className == "pyret")? "pyret"
            : false;
    if(mode && tt.firstChild){
      CodeMirror.runMode(tt.firstChild.nodeValue, mode, node);
    }
...
window.addEventListener("load", function(){
  ...
  initializeCards();
  rewrapCodeExps();
  setTimeout(function(){window.status = 'printready';}, 250);
});
