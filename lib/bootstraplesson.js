...
window.addEventListener("load", function(){
  ...
  var circles = document.getElementsByClassName('circevalsexp'),
      sexps   = document.getElementsByClassName('sexp');
  for(var i=0; i<circles.length; i++){
    circles[i].onclick = function(){this.className = (this.className=="sexp")? "circevalsexp" : "sexp";};
  }
  for(var i=0; i<sexps.length; i++){
    sexps[i].onclick = function(){this.className = (this.className=="sexp")? "circevalsexp" : "sexp";};
  }
