...
@lesson/studteach[
     ...
     #:pacings (list 
        ...
        @points[
                @point{@student{Open the programming environment: @hyperlink["https://code.pyret.org" "click here to log in"].
                                Log in using your Google account, then click "New File," name it "expressions", and click "Create".}
                ...
                @point{@student{The Circles of Evaluation are also easy to convert into computer programs. To translate a Circle of
                                Evaluation that uses arithmetic into a program, begin with an open parenthesis @code{(}, and then translate the expression on the left, followed by the operation written at
                ...
                @;{ NOTE(joe): It's unfortunate that the below is lost. Make a
                bigger deal about Pyret forcing parens as a result? }

                @;{@bannerline{All of the expressions that follow the function name are called @vocab{arguments} to the 
                ...
                @point{@student{When expressions don't follow the code rules, the computer will tell you that it found a problem.  The
                       ...
                       @activity[#:forevidence "BS-IDE&1&2"]{Enter each of the following illegal expressions in the Interactions area 
                                 ...
                                 @itemlist[@item{@code{(* 5 6)} [puts the operator in the front, instead of in the middle]}
                                           @item{@code{(5 *6)}  [missing a space after the operator]}
                                           @item{@code{5 * 6)}  [forgets the open parenthesis]}
                                           @item{@code{(5 * 6}  [forgets the close parenthesis]}
                                           @item{@code{5 * 6 - 1}  [no parentheses at all]}
                                          ]}}
