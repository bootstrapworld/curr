...
@lesson/studteach[
     ...
     #:pacings (list 
        ...
        @points[@point{@student{What datatypes have you seen so far? Can you think of @code{Number} values? @code{String} values? @code{Image} values?
     ...
     #:pacings (list 
        ...
        @points[@point{@student{@activity[#:forevidence (list "BS-IDE&1&1")]{What would each of the following expressions @vocab{evaluate} to? (If you're not sure, you can type it in and try it out!)
                 ...
                 @point{@student{The last expression, @code{(3 < 4)},
                                                      uses a new operator that @italic{compares Numbers}, returning
                 ...
                 @point{@student{Operators like @code{<}, @code{>} and @code{==} all consume two Numbers and produce a special type of value called a 
                         @vocab{Boolean}. Booleans are answers to a yes-or-no question, and Boolean functions are used to perform tests. 
                 ...
                 @point{@student{There are functions that produce Booleans as well. Here's one that can be used to compare two @vocab{Strings}: @code[#:multi-line #t]{string-equal("apples", "oranges")}}
     ...
     #:pacings (list 
        ...
        @points[@point{@student{Open the "Cage" program [@(new-tab "https://code.pyret.org/editor#share=1B5T5dTyRPb3rKJBZOgbYbQcFcsDeYZAA" "code.pyret org")] on your computer, and click "Run". The window that pops up contains a butterfly, which you can move around using the arrow keys on your keyboard. This butterfly is named Sam, and the window is Sam's yard. Much like the videogame screen, this yard is 640 pixels wide by 480 pixels tall! As you can see, Sam is free to walk around the yard, and the coordinates at the top of the screen update to tell you his location.}
                 ...
                 @point{@student{Unfortunately, right now there is nothing to keep Sam from leaving his yard! Looking at the code for this program, we find three function definitions:
                                  @itemlist[@item{@code{is-safe-left}, which takes in Sam's x-coordinate and checks to see if Sam is safe on the lefthand side of the screen. Right now, this function always returns @code{true}.}
                                             @item{@code{is-safe-right}, which takes in Sam's x-coordinate and checks to see if Sam is safe on the righthand side of the screen. Right now, this function always returns @code{true}.}
                                             @item{@code{is-onscreen}, which takes in Sam's x-coordinate and checks to see if at least tiny piece of Sam is on the screen. Right now, this function @italic{asks @code{is-safe-left} if the given x-coordinate is safe.}}]
                         ...
                         @teacher{It's extremely valuable at this point to have three students stand, and act out each of these three functions:
                                  @itemlist[@item{Ask each student to tell you their Name, Domain and Range. If they get stuck, remind them that all of this information is written in their Contract!}
                                             @item{Practice calling each function, by saying their name and then giving them an x-coordinate. For example, "is-safe-left fifty" means that the number @code{50} is being passed into @code{is-safe-left}. That student should return "true", since the code currently returns @code{true} for all values of x.}
                                             @item{Do this for all three functions, and have the class practice calling them with different values as well.}
                                             @item{Note: the volunteer for @code{is-onscreen} should first call @code{is-safe-left}, before replying with the value.}]}
                         }
                 @point{@student{Right now @code{is-safe-left} is ignoring Sam's x-coordinate, and blindly returning @code{true} no matter where he is. 
                                           ...
                                           @activity[#:forevidence (list "BS-DR.1&1&1" "BS-DR.2&1&1" "BS-DR.2&1&3" "BS-DR.3&1&1" "8.F.1-3&1&1" "F-IF.1-3&1&1")]{
                                                 ...
                                                 function, so that @code{is-safe-left} takes in Sam's x-coordinate and returns @code{true} or @code{false} 
                                                 depending on whether it is @italic{greater than -50}.}
                                           Once you have typed in the examples and definition for @code{is-safe-left}, click "Run" and see if Sam is prevented 
                                           from flying off the lefthand side of the screen. Be sure to test your function in the Interactions window as well, 
                                           for example:
                                  @code[#:multi-line #t]{is-safe-left(50)
                                                         is-safe-left(-100)
                                                         is-onscreen(50)
                                                         is-onscreen(-100)}
                                  }
                         @teacher{Remind students that they've already written the check for this on page 19! 
                                 ...
                                 Have the three volunteers from earlier stand again, and act out the code now that @code{is-safe-left} is written properly. As before, make sure students practice calling each function individually: now @code{is-safe-left} will be able to return true and false (as will @code{is-onscreen}), while @code{is-safe-right} still returns only @code{true}.}
                         }
                 @point{@student{@activity[#:forevidence (list "7.EE.3-4&1&1" "7.EE.3-4&1&3" "7.EE.3-4&1&4" "7.EE.3-4&1&5" "7.EE.3-4&1&10" "F-IF.4-6&1&1")]{
                                     ...
                                     so that @code{is-safe-right} takes in Sam's x-coordinate and returns @code{true} or @code{false} depending on whether it 
                                     is @italic{less than 690}.
                                     }
                                  }
                         @teacher{After walking through @code{is-safe-right} section by section, students should be able to complete this activity on their own. 
                 ...
                 @point{@student{Once you have typed in the examples and definition for @code{is-safe-right}, click "Run" and see if Sam is prevented from flying off the righthand side of the screen. You may be surprised at what happens! Test your function in the Interactions window:
                                  @code[#:multi-line #t]{is-safe-right(50)
                                                         is-safe-right(810)
                                                         is-onscreen(50)
                                                         is-onscreen(810)}
                         Is @code{is-safe-right} working the way you expected?  Is @code{is-onscreen} working as you expected?
                         The @code{is-onscreen} function is only talking to @code{is-safe-left}, which means that Sam is only protected on the lefthand side of the screen.
                          @activity{Change the code for @code{is-onscreen} so that it calls @code{is-safe-right} instead. 
                                   Now Sam will be protected on the righthand side of the screen, but not the left. What should @code{is-onscreen} be doing?}}
                         @teacher{Have the three actors stand again, and repeat the activity. Now @code{is-safe-left} and @code{is-safe-right} behave correctly, but @code{is-onscreen} is still @bold{only talking to @code{safe-left}!}}
                         }
                 ]}
...
@lesson/studteach[
     #:title "and/or"
     #:duration "10 minutes"
     #:overview "Students are introduced to and/or, two operators that consume Booleans"
     ...
     #:pacings (list 
       ...
       @points[@point{@student{You've already learned several operators that allow you to create Booleans. The operators @code{<} and @code{>}, for example, will each return a Boolean when applied to the appropriate input. What happens when you want to @italic{combine} these tests, to see if more than one thing is true? 
                ...
                @point{@student{@code{and} and @code{or} are two new operators.
...
@lesson/studteach[
     ...
     #:materials @itemlist[@item{Pens/pencils for the students, fresh whiteboard markers for teachers}
                            ...
                            @item{Language Table (see below)}]
     #:preparation @itemlist[]
     #:prerequisites (list "is-onscreen 1.0" "and/or")
     #:pacings (list 
                @pacing[#:type "remediation"]{@itemlist[@item{}]}
                @pacing[#:type "misconception"]{@itemlist[@item{}]}
                @pacing[#:type "challenge"]{@itemlist[@item{}]}
       ...
       @points[@point{@student{Now that you know how to combine tests, how can @code{is-onscreen} combine both tests to keep Sam is-onscreen
                               @activity[#:forevidence (list "BS-DR.2&1&1" "BS-DR.2&1&3" "BS-DR.3&1&1" "F-IF.4-6&1&3" "A-SSE.1-2&1&4" "A-CED.1-4&1&1")]{
                                    Turn to @worksheet-link[#:name "onscreen"] in your workbook and write the completed @code{is-onscreen} 
                                    function, so that it tests both @code{is-safe-left} and @code{is-safe-right}. Type in the examples and definition when you're done,
                                    click "Run", and see if Sam is protected on both sides!}
                               }
                       @teacher{Have the volunteers stand once again, and ask the class what @code{is-onscreen} @italic{should} do. Should it be using @code{and} or @code{or}? Be sure to give specific examples of points that are onscreen, too far to the left and too far to the right, and challenge the students to justify their answers.}
                       }
                @point{@student{There are two ways to go about solving the @code{is-onscreen} word problem: 
                                @code[#:multi-line #t]{fun is-onscreen(x)
                                                         (x > -50) and (x < 690)
                                                       end}
                                and...
                                @code[#:multi-line #t]{fun is-onscreen(x)
                ...
                @point{@student{The boundary-detection code you wrote (through @code{is-safe-left} and @code{is-safe-right}) is very useful for videogames. It can be used to stop a character from traveling through a wall, or to regenerate the character once it has left the screen. @activity{Open your game file, and scroll down past the @code{update-} functions until you see the definitions for @code{is-safe-left}, @code{is-safe-right} and @code{is-onscreen}.  Right now, they all return true. Since your gamescreen has the same dimensions as Sam's yard (640x480), you can use the same code for each of these three functions! Make sure you include examples, so that your game code can test each function separately.}}
                       @teacher{}
                       }
                ]}
