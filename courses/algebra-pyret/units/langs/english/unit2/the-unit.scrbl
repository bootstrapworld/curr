...
@lesson/studteach[
     #:title "Functions, Strings, and Images"
     #:duration "20 minutes"
     ...
     #:pacings (list 
        ...
        @points[          
            @point{@student{The Circles of Evaluation are a powerful tool, and can be used for much more than just numbers.
                            ...
                            For this Circle, there is a name, @code{star}, rather than an operator (like @code{+} or @code{-}) at the top. We say that this kind of circle uses a @vocab{function} name at the top, rather than an operator.
                            ...
                            parenthesis @code{(}, followed by the expressions
                            in the circle @bold{separated by commas}, followed
                            by a close parenthesis @code{)}.  The example above
                            would translate to

                            @code[#:multi-line #t]{star(50, "solid", "red")}

                            We say that the expressions separated by commas are
                            the @vocab{arguments} that are given to the
                            function.

                            

                            @activity[#:forevidence "N-Q&1&2"]{@itemlist[
                                                 @item{How many arguments are being given to the function?}
             ...
             @point{@student{This expression also included a new @vocab{function} called @code{star}. Similar to how the addition operator @code{+} takes in two Numbers, @code{star} takes in @italic{a Number and two Strings}, and produces a new type of data, called an @vocab{Image}. The way we use functions is different than operators (name, then arguments separated by commas in parentheses), but they do similar things.
             ...
             @point{@student{You've also seen expressions that @vocab{produce} values, such as @code{(16 * 4)}, which produces a Number. Other expressions, however, can produce Strings or Images. 
                             @activity[#:forevidence "BS-PL.1&1&2"]{What @bold{type} of data will each of the expressions listed below evaluate to?
             ...
             @activity{@itemlist[
                ...
                @item{Draw the circle of evaluation that corresponds to this expression.}
             ]}
             }
...
@lesson/studteach[
     ...
     #:pacings (list 
        ...
        @points[@point{@student{Now we've seen two functions: @code{num-sqr}, which takes in a number and produces a number, and @code{star}, which takes in a Number and two Strings, and produces an Image.
                 ...
                 @point{@student{Domains and Ranges help programmers write better code, by preventing silly mistakes and giving themselves 
                                 ...
                                 like @code{(25 * 4)}. We know this code will evaluate to an appropriate value (Number) since its an operator, therefore, the result of @code{*} can be used in place of any Number value.}
                         @teacher{}
                         }
             ...
             @point{@student{By writing down the Contracts for our functions, we can easily look back to see how they are used.
                             @activity[#:forevidence (list "BS-PL.2&1&1" "F-IF.1-3&1&1")]{The Contract for @code{num-sqr} is shown below.
                                       @code[#:multi-line ""]{num-sqr :: Number -> Number}                
                                       Can you write the Contract for @code{num-sqrt}, which takes the square root of a number?}}
                     @teacher{}
                     }
             @point{@student{Now that you know how to use a Contract to write an expression, here are the Contracts for several new functions that produce Images:
                             ...
                             @activity[#:forevidence (list "BS-PL.2&1&1" "BS-PL.2&1&3")]{See if you can figure out how to use these new functions to draw
                                                                                         other shapes! Here's an example to get you started: @code{ellipse(50, 100, "solid", "blue")}]}
                             }
             ...
             @point{@student{Contracts help programmers write code, so it's always a good idea to write down contracts for each function you see. 
                             @activity[#:forevidence (list "BS-PL.2&1&2" "F-IF.1-3&1&1")]{
                                  ...
                                  some sample code?  Look at the function being used here: @code{text("Bootstrap", 30, "purple")}, 
                ...
                @point{@student{Often, we make mistakes when we write code, and we use a value that violates the contract.  Fortunately,
