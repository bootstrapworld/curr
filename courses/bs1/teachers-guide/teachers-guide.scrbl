...
@itemlist/splicing[@item{Near the end of the game file, you'll see an expression that defines a level, @code{(define game_level ...)}, passing in animation functions and images. You can define other levels if you wish, with different images or animation functions.}
                    @item{The last line, @code{(play game_level)} will run the game using the level created above. By default, this line will be executed every time the student clicks "Run". You can stop this by adding a semicolon at the beginning of the line: @code[#:contract "(play game_level)"]. This can make it easier for student to experiment with their code and test cases while they're developing, and they can always run the program manually by typing @code{(play game_level)} in the Interactions window}]}
...
@itemlist/splicing[@item{Open the Game Template file for a team of students.}
                   ...
                   @item{At the moment, they are defined to be simple images. For example: @linebreak[]
                         @code{(define BACKGROUND (rectangle @code{640 480} @code{"solid" "black"}))}}
...
@code{(define BACKGROUND (bitmap @code{"tami+chris/water.png"}))}}
                   @item{@bold{In WeScheme (or DrRacket 5.1)}, replace bitmap with bitmap/url, and the full URL of their images as a String: @linebreak[]
@code{(define BACKGROUND (bitmap/url @code{"http://exampleschool.edu/tami+chris/water.png"}))}}
                   ...
                   @item{You can rotate, scale, or flip the images using Racket functions: @linebreak[]
                         @code[#:contract "rotate : Number Image -> Image"] @linebreak[]
                         @code[#:contract "scale : Number Image -> Image"] @linebreak[]
                         @code[#:contract "scale/xy : Image Number -> Image"] @linebreak[]
                         @code[#:contract "flip-horizontal : Image -> Image"] @linebreak[]
                         @code[#:contract "flip-vertical : Image -> Image"]}]}
...
@itemlist/splicing[@item{Every Bootstrap game includes at least three "characters": the @code{Player}, the @code{Target} and the @code{Danger}.}
                   ...
                   @item{These functions take the current coordinate as input, and return the next coordinate. If the @code{Target} moves to the left by 20 pixels each frame, then the body of the function should return @code{(- x @code{20})}.}
                   ...
                   @item{If you'd like the player to move differently depending on which key is pressed, you'll need to use @code{cond}. For example: @linebreak[]
                        @code[#:contract "update-player : Number String -> Number"] @linebreak[]
                        @code[#:purpose "given the y-coordinate and a direction, output the NEXT y"]
                        @code[#:multi-line ""]{(define (update-player y key)
                                                 (cond
                                                   [(string=? key "up")   (+ y 10)]
...
@itemlist/splicing[@item{The teachpack will automatically regenerate the characters if the @code{onscreen} function returns @code{false} when given the character's x- and y-coordinates.}
                   ...
                   @item{In the lesson plans, students write functions that check the left and right sides of the screen separately: @linebreak[]
                    @code[#:contract "protect-left? : Number -> Boolean"] @linebreak[]
                    @code[#:purpose "Determines if the x-coordinate is greater than -50"]
                    @code[#:multi-line ""]{(define (protect-left? x)
                                             (> x -50))}
                    @code[#:contract "protect-right? : Number -> Boolean"] @linebreak[]
                    @code[#:purpose "Determines if the x-coordinate is less than 690"]
                    @code[#:multi-line ""]{(define (protect-rigt? x)
                                             (> x 690))}}
                    @item{Then they write a single function, called @code{onscreen?}, which is only true if their characters are protected on the left AND the right: @linebreak[]
                     @code[#:contract "onscreen? : Number Number -> Boolean"] @linebreak[]
                     @code[#:purpose "Determines if the coordinates are within 100 pixels of the screen"]
                     @code[#:multi-line ""]{(define (onscreen? x)
                                              (and (protect-left x)
                                                (protect-right x)))}}
...
@itemlist/splicing[@item{To detect whether or not two characters have collided, the game code needs to know two things: how close they are, and how close they are allowed to be before a "collision" occurs.}
                   ...
                   @item{The lengths of each the legs is calculated by the @code{line-length} function. Modify the body so that it correctly calculates the distance between @code{a} and @code{b}. (You can do this using @code{abs} or @code{cond}:@linebreak[]
                    @code[#:contract "line-length : Number Number -> Number"] @linebreak[]
                    @code[#:purpose "the distance between two points on a number line"]
                    @code[#:multi-line ""]{(define (line-length a b)
                                             (abs (- a b)))}}
                   @item{The hypoteneuse length is calculated by the @code{distance} function. Students will code the distance formula here, using the @code{line-length} function:@linebreak[]
                    @code[#:contract "distance : Number Number Number Number -> Number"] @linebreak[]
                    @code[#:purpose "the distance between two points on a screen"] @linebreak[]
                    @code[#:purpose "We have the player's x and y, and a character's x and y."] @linebreak[]
                    @code[#:purpose "How far apart are they?"]@code[#:multi-line ""]{(define (distance px py cx cy)
