...
@lesson/studteach[
     ...
     #:pacings (list 
        ...
        @points[
                ...
                @point{
                      @student{
                              You may have noticed that answering these questions requires a little
                              extra work, because the columns involved are far apart.  Finding the political
                              party for Franklin Pierce requires looking at the @code{name, party} columns, but
                              not any of the columns in between them.  It would be nice if we could 
                              choose only the important columns and put those into a new table.
                ...
                @point{
                      @student{
                              The following code will select only the @code{name, rating} columns from the restaurants table,
                              and bind the new table to the variable @code{restaurants-selected}:
                                ...
                                @activity[#:forevidence "BS-IDE&1&1"]{
                                        @itemlist[
                                                @item{
                                                        ...
                                                        @code{countries-selected} by selecting the @code{country} and @code{continent} 
                                                        columns from the @code{countries} tables.
                                                }
                                                @item{
                                                        In your definitions window, write code to create a new table with the variable name
                                                        @code{nutrition-selected} by selecting the @code{food} and @code{fat} columns from @code{nutrition}.
                                                }
        ...
        @points[
                @point{
                        @student{
                                ...
                                The expression for ordering the @code{nutrition} table is the following:
                ...
                @point{
                        @student{
                                ...
                                The key word @code{order} is followed by the name of the table we are ordering 
                                (in this case, @code{nutrition}), then a colon (@code{:}).  The colon always 
                                comes in front of an expression using column names.  In this case, we are ordering
                                by the @code{sodium} column, in @code{descending} order.

                                @activity[#:forevidence "BS-IDE&1&1"]{
                                        Change the keyword @code{descending} in the 
                                        definition of @code{nutrition-ordered} to @code{ascending}.   Then
                                        hit Run and type @code{nutrition-ordered} into the interactions window.
                                        What has changed?
                                }
                                ...
                                @activity[#:forevidence "BS-IDE&1&1"]{
                                        @itemlist[
                                                ...
                                                @item{
                                                    ...
                                                    Create a new table @code{presidents-ordered} that is the @code{presidents}
                                                    table ordered by the @code{nth} column, in @code{ascending} order.
                                                    What are the political parties of the first 5 presidents?
                                                }
                                                @item{
                                                    Create a new table @code{countries-ordered} that is the @code{countries} table 
                                                    ordered by the @code{population} column, in descending order. 
                                                    What are the 3 countries have the highest population?
                                                }
                ...
                @point{
                        @student{
                                ...
                                to see the first 5 presidents' political parties, or the 3 most populated countries.              
                        }
                        @teacher{
        ...
        @points[
            @point{
                    @student{
                            ...
                            This new table was created using both the @code{order} and @code{select}
                            table operations.
                    }
                    @teacher{
            ...
            @point{
                    @student{
                            @activity[#:forevidence "BS-IDE&1&1"]{
                                    Write the code necessary for each of these in the definitions window.  Make 
                                    sure to comment out the select statements you wrote earlier for exercises 1, 2.

                                    @itemlist[
                                            @item{
                                                    Create a new table containing only the @code{name} column of @code{presidents-ordered}, 
                                                    and bind the variable @code{nutrition-selected} to this table.
                                            }
                                            @item{
                                                    Select @code{country} column from the @code{countries-ordered} table, 
                                                    and bind this table to the variable @code{countries-selected}.
                                            }
                                    ]
                            }
                    }
                    @teacher{
                            Students must comment out their earlier code to avoid name clashes between @code{nutrition-selected}
                            and @code{countries-selected}.  We use the same names in different contexts, because this 
                            naming scheme helps scaffold students later when breaking down problems into their different table operations.
                    }
            }
        ]
   }
