...
@activity[#:forevidence (list "6.NS.5-8&1&4")]{When the cat and dog were switched, did you still subtract the dog's position from the cat's, or subtract the cat's position from the dog's? Why?}}
                 ...
                 @point{@student{Unfortunately, most distances aren't only measured in one dimention. We'll need some code to calculate the distance between two points in two dimentions, in case two characters don't happen to be on the same x or y-axis.                   
...
You still need a function to check whether or not two things are colliding.}
                ...
                @point{@student{Now that you have a function which will check whether two things are colliding, you can use it in your game! For extra practice, You can also implement collision detection into this @editor-link[#:public-id "0B9rKDmABYlJVT1FBd3RpQWFqbGM" "Pyret Ninja Cat game"]. This is the program we'll be altering for this lesson, as an example. 
                                @activity[#:forevidence (list "BS-W&1&4")]{Out of the major functions in the game (@code{next-state-tick}, @code{draw-state}, or @code{next-state-key}), which do you think you'll need to edit to handle collisions?}
                                We'll need to make some more @code{if} branches for @code{next-state-tick}. In Ninja Cat, when the cat collides with the dog, we want to put the dog offscreen so that he can come back to attack again.}
                        @teacher{}}
...
@code[#:multi-line #t]{if is-collision(
             ...
             @code[#:multi-line #t]{if is-collision(
                  ...
                  @student{@activity[#:forevidence (list "BS-PL.4&1&1" "BS-DR.4&1&3" "BS-DS.1&1&5")]{Once you've finished, write another branch to check whether the player and the target have collided. @bold{Challenges:} @itemlist[@item{Change your first condition so that the danger gets reset only when the player and danger collide @italic{AND} the cat is jumping. (What must be true about the player's y coordinate for it to be jumping?)}
                            @item{Add another condition to check whether the player has collided with the danger while the player is on the ground. This could be a single expression within @code{next-state-tick}, or you can write a @vocab{helper function} called @code{game-over} to do this work, and use it in other functions as well (maybe the GameState is drawn differently once the game is over.)}] 
                                }}
