...
@lesson/studteach[#:title "Protecting the Boundaries"
        ...
        @points[@point{@student{Just like in Bootstrap 1, we need to write a function that checks whether the dog has gone off the right side 
...
@code[#:multi-line #t]{; > : Number Number -> Boolean
                 ...
                 @point{@student{@activity{@itemlist[@item{Turn to @worksheet-link[#:page 33 #:name "Boundary Checks"] in your workbook.}
                                                      ...
                                                      @item{@code{off-right?} will return @code{true} if a character goes off the right side 
                                                             of the screen. So, it will need to check if their x-coordinate is greater 
                                                             than what number? (Hint: How large is the game screen?)}
                     ...
                     ; returns true if the given number is greater than 640}
                                 ...
                                 @activity{@itemlist[@item{What is an x-coordinate that would put a character at the center of the screen?}
                                                      ...
                                                      @item{Any x-coordinate greater than 640 is off the right side of the screen, so how 
                                                            would you check if the example number is @italic{greater than} 640?}]}
		...
		[else  (make-world  (+ (world-dogX w) 10) 
				    (- (world-rubyX w) 5)
				       (world-catY w))]))}
                                  ...
                                  @code{off-right?} @italic{does the work for you}. But how would you check if the dog is off the right? You'll 
                                  need to pull the dog's x-coordinate out of the world... 
		...
		[else  (make-world  (+ (world-dogX w) 10) 
				    (- (world-rubyX w) 5)
				       (world-catY w))]))}
                                  ...
                                  of the screen.
                                  @activity{@itemlist[@item{What will the new value of @code{dogX} be, if it it moves back to the 
		...
		[else  (make-world  (+ (world-dogX w) 10) 
				    ...
				    (world-catY w))]))}}
                         @teacher{This can be an opportunity to discuss abstraction, and the usefullness of reusing code, with your
                                  ...
                                  compared to 640. Additionally, from a programming perspective, it makes sense to use the 
                                  specific screen boundaries in as few functions as possible: If in the future, a programmer wants 
                                  their game to be playable on a larger screen (such as a tablet), they must then go through their
...
@lesson/studteach[#:title "Tests and Results"
        ...
        @points[@point{@student{Now to use what you know about boundary detection, and apply it to your own game. 
                                @activity{@itemlist[@item{Open your game file.}
                                                     @item{The first thing to do is to reformat your @code{update-world} function 
                                                           so that it uses @code{cond}, with your current code inside the 
                                                           @code{else} clause.}
                                                     ...
                                                     @item{On @worksheet-link[#:page 34 #:name "Test and Result"], make a
                                                           list of the tests that you need to do in the left hand column, 
                                                           to decide whether each thing flies offscreen. For example, 
                                                           with the dog we said @code{(off-right? (world-dogX w))}. On the
                                                           right, figure out which world you need to make, so that the thing
...
@lesson/studteach[#:title "Closing"
        ...
        @points[@point{@student{Take a minute and admire your handiwork: You've put a lot of time and effort into your 
                                ...
                                collides with the dog, or ruby. In the next unit we'll change that, and you'll be able
