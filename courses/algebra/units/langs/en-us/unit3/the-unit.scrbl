...
@lesson/studteach[
     ...
     #:materials @itemlist[@item{"Algebra Translation" [@resource-link[#:path "source-files/Algebra.rkt" #:label "DrRacket"] | @(hyperlink "https://www.wescheme.org/openEditor?publicId=s2s0tkTgeF" "WeScheme")] preloaded on students' machines}]
     #:preparation @itemlist[]
...
@lesson/studteach[
     ...
     #:pacings (list 
        ...
        @points[@point{@student{@activity{Open the videogame file (Game.rkt from @resource-link[#:path "source-files.zip" #:label "source-files.zip"]
                ...
                @point{
                    @student{
                        Just like all digital images and video, your video game and its @code{SCREENSHOT} are made up of @vocab{pixels}, a series of small, specifically-colored dots or squares. When we talk about coordinates in our video game, we measure them in terms of pixels. Therefore, the code @code{(put-image PLAYER 320 240 BACKGROUND)} will place the image of the player 320 pixels to the @italic{right} of the origin (lower-left corner), and 240 pixels above the origin. Since the background image itself is 640x480 pixels total, this will place the @code{PLAYER} in the center of the background.
                    }
                    @teacher{
                        Be sure to remind students that @code{SCREENSHOT} is @italic{NOT} the same thing as the video game itself. @code{SCREENSHOT} is a static image representing the characters at specific coordinates. Changing the coordinates of characters within the @code{SCREENSHOT} definition has no effect on the game itself.
                    }
                }
