...
@lesson/studteach[
     ...
     #:pacings (list 
       ...
       @points[@point{@student{Suppose two objects are moving through space, each one having its own (x,y) coordinates.
                                       When do their edges start to overlap? @slidebreak They certainly overlap if their coordinates are identical (x1=x2, y1=y2),
                ...
                @point{@student{@bitmap{images/numberline.png}In one dimension, it's easy to calculate when two objects overlap. In this example,
                                       the red circle has a radius of 1, and the blue circle has a radius of 1.5.@slidebreak
                ...
                @point{@student{Your game file provides a function called @code{line-length} that computes the difference between two points on a number line. @slidebreak
                       ...
                       @activity[#:forevidence (list "BS-PL.3&1&3" "BS-PL.4&1&1" "6.NS.5-8&1&6")]{What answers would you expect from each of the following two uses of @code{line-length}:
			                   ...
			                   @item{@code{(line-length 5 2)}}
                                 ...
                                 Do you expect the same answer regardless of whether the larger or smaller input goes first?@slidebreak}}
                       @teacher{If you have time and want to reinforce how conditionals arise from examples, you can have students fill in blanks in Examples such as
                                   @code{(EXAMPLE (line-length 2 5) ___)}, circle what's different, and notice that the circle labels are in different orders
                                   depending on whether the first or the second input is larger.
                                   This in turn suggests that the code for @code{line-length} will need a conditional.
                                   In this case, one could avoid the conditional by taking the absolute value of the difference
                                   (the function @code{abs} does this); if you are working with older students who already know about absolute value you could show it.
                                   Using @code{cond}, however, emphasizes how code structure arises from examples.}}

                @point{@student{Scroll to the @code{line-length} and @code{collide?} functions in your game file. @slidebreak
                                       Notice that @code{line-length} uses a conditional so that it subtracts the smaller number from the bigger one. @slidebreak
                ...
                @point{@student{@bitmap{images/3004graph.png}Unfortunately, @code{line-length} can only calculate the distance between points in a single dimension (x or y).
                                       How would the distance be calculated between objects moving in 2-dimensions (like your game elements)? @code{line-length} can
                                       calculate the vertical and horizontal lines in the graphic shown here, using the distance between the x-coordinates and the distance
                                       between the y-coordinates. Unfortunately, it doesn't tell us how far apart the two centers are.}
                       @teacher{}
                       }
                @point{@student{@bitmap{images/3004ABCgraph.png}Drawing a line from the center of one object to the other creates a right-triangle, with sides A, B and C.@slidebreak
                                       A and B are the vertical and horizontal distances, with C being the distance between the two coordinates.
                                       @code{line-length} can be used to calculate A and B, but how can we calculate C?}
                       @teacher{Students' gamefiles all have a value called @code{*distances-color*}, which is set to the empty string @code{""}.
                                        By changing this to a color such as "yellow" or "red", the game will draw right triangles between each game character,
                                        and fill in the lengths for each side. You may want to demonstrate this using your own game file, and have the students follow along.
                                        Hint: to make it as easy as possible to see these triangles, set your background to be a simple, black rectangle and slow down the animation functions.}
                       }
                @point{@student{In a right triangle, the side opposite the 90-degree angle is called the @vocab{hypotenuse}. @slidebreak
                                   Thinking back to our collision detection, we know that the objects will collide if the hypotenuse is @italic{less than the sum of their radii}.
                                   Knowing the length of the hypotenuse will be essential to determine when a collision occurs.}
                       @teacher{}
                       }
                ]}
...
@lesson/studteach[
     ...
     #:pacings (list 
       ...
       @points[@point{@student{Ancient civilizations had the same problem: they also struggled to find the distance between points in two dimensions!
                ...
                @point{@student{@bitmap{images/csquared.png}For any right triangle, it is possible to draw a picture where the hypotenuse is used for all four sides of a square.@slidebreak
                                       In the diagram shown here, the white square is surrounded by four gray, identical right-triangles, each with sides A and B.
                                       The square itself has four identical sides of length C, which are the hypotenuses for the triangles.
                ...
                @point{@student{@bitmap{images/absquare.png}The smaller square has an area of @math{A^{2}}, and the larger square has an area of @math{B^{2}}. @slidebreak Since these squares are
                                       just the original square broken up into two pieces, we know that the sum of these areas must be equal to the area of the original square:
                ...
                @point{@student{The code on @worksheet-link[#:name "distance-example"] will accurately calculate the distance between two objects whose centers are at (3,0) and (0,4).@slidebreak 
                                    But what about other points? It would be nice to have a function that calculates the distance for @italic{any} two sets of points.
...
@lesson/studteach[
     ...
     #:pacings (list 
       ...
       @points[@point{@student{By now, you have defined a function called @code{distance}: it has four Number inputs (representing playerX, playerY, dangerX and dangerY) and
                                  produces a Number representing the distance between those coordinates. @slidebreak If the player is standing at (320, 240) and the danger is at (400, 159),
                ...
                @point{@student{Now that you know how to compute the distance between two objects, you need to decide when two objects have collided. @slidebreak
                                    We discussed this earlier in the unit using circles: two circles collide when the distance between them is less than the sum of their radii. @slidebreak
                                    If your objects have more interesting outlines than circles, this computation can be hard to do precisely.  Fortunately, most games don't have to be precise
                                    (they move too fast for people to see the exact moment of impact).  We just need to figure out when items are close enough, and use that to detect collision.}
                       @teacher{You can spend additional time on this point by having students think about collision distances between different shapes. @slidebreak
                                    The rest of Bootstrap doesn't require this, but this may be a good point to integrate additional geometry if you are teaching math.}}
                @point{@student{Unless the images you chose for your game are very small, 50 pixels is usually a sufficient distance for detecting collisions between your player and danger.@slidebreak
                                       If you are using very small images, you might want to detect a collision when the distance between characters is below 20.
                              @activity[#:forevidence (list "8.G.6-8&1&3")]{How would you check whether the distance between (320, 240) and (400, 159) is @italic{less than 50?}
                                                                                How would you check whether the distance between those coordinates is less than 20?}}
                       @teacher{}
                       }
                @point{@student{@activity[#:forevidence (list "8.F.1-3&1&1" "8.F.1-3&1&3")]
                                         {Turn to @worksheet-link[#:name "collide"] in your workbook, and use the Design Recipe to write a function that produces @code{true}
                                               if the distance between two coordinates is less than 50. HINT: You should use your @code{distance} function!}
                                @activity{Enter your @code{collide?} function definition into your game file.  Play your game, and make your player collide with the danger.
                                                Does your game now do something different than it did before you wrote @code{collide?}?}
                      }
                        @teacher{}
                        }
                ]}
...
@lesson/studteach[
     ...
     #:pacings (list 
        ...
        @points[@point{@student{Congratulations - your game is complete! Take a minute to scroll through all the code you've written, and think
                                back on how much you've learned: @slidebreak The Circles of Evaluation, datatypes like @code{Number}, @code{String}, @code{Image}
                                and @code{Boolean}, the importance of Contracts and Purpose Statements, and Piecewise functions. @slidebreak You've learned a whole 
                 ...
                 @point{@student{@activity{Once programmers get something working, they always go back over their code and make sure that it's readable, 
                                          ...
                                          if it's impossible for a mechanic to repair or upgrade! @slidebreak Make sure your code is beautiful, inside and out.
